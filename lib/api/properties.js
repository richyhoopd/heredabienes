...
// Destacadas para el Home. Si no hay suficientes marcadas como destacadas,
// completa con las publicadas más recientes: es preferible mostrar el catálogo
// lleno a dejar huecos en la portada por un check que nadie marcó.
export async function listFeaturedProperties(limit = 3, client) {
  const supabase = resolveClient(client);

  const { data: destacadas, error } = await supabase
    .from('properties')
    .select(SELECT)
    .eq('publicado', true)
    .eq('destacado', true)
  ...
  if (destacadas.length >= limit) return destacadas.map(fromRow);

  const yaIncluidas = destacadas.map((p) => p.id);
  let relleno = supabase
    .from('properties')
    .select(SELECT)
    .eq('publicado', true)
    .eq('destacado', false)
    .order('orden', { ascending: true })
    .order('created_at', { ascending: false })
    .limit(limit - destacadas.length);
  if (yaIncluidas.length) relleno = relleno.not('id', 'in', `(${yaIncluidas.join(',')})`);

  const { data: recientes, error: errorRelleno } = await relleno;
  if (errorRelleno) throw errorRelleno;

  return [...destacadas, ...recientes].map(fromRow);
}
...
// Verifica el slug antes de guardar: consulta directa por índice único en
// lugar de traer todo el catálogo. `idExcluir` permite reutilizar el mismo
// slug al actualizar la propiedad que ya lo tiene.
export async function slugDisponible(slug, idExcluir, client) {
  const supabase = resolveClient(client);
